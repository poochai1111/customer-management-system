...
public class EmailDeleteServlet extends HttpServlet {
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		boolean mark = service.delete(emailId);
		if(mark){
			request.setAttribute("info", "邮件删除成功");
		}else{
			request.setAttribute("info", "邮件删除失败");
		}
		//转发
		request.getRequestDispatcher("/view/email/email_save.jsp").forward(request, response);
	}
